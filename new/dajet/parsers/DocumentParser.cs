    ...
    internal sealed class DocumentParser : ConfigFileParser
    {
        internal override void Parse(Guid uuid, ReadOnlySpan<byte> file, in MetadataRegistry registry)
        {
            ...
            if (reader[2][10][2][3].Seek())
            {
                ...
                metadata.Name = name;
                registry.AddMetadataName(MetadataName.Document, in name, uuid);
            }
        ...
        private void AddRecorderToRegisters(ref ConfigFileReader reader, Guid document, in MetadataRegistry registry)
        {
