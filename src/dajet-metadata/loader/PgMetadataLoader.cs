...
namespace DaJet.Metadata
{
    internal sealed class PgMetadataLoader : MetadataLoader
    {
        ...
        private const string PG_STREAM_SCHEMA_STORAGE = "SELECT (CASE WHEN SUBSTRING(currentschema, 1, 3) = E'\\\\xEFBBBF' THEN 1 ELSE 0 END) AS UTF8, LENGTH(currentschema) AS DataSize, schemaid::char(1) AS FileName, currentschema AS BinaryData FROM schemastorage WHERE schemaid > 0 AND status = 100;";
