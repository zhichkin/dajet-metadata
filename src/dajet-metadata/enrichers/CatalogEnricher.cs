...
namespace DaJet.Metadata.Enrichers
{
    public sealed class CatalogEnricher : IContentEnricher
    {
        ...
        public void Enrich(MetadataObject metadataObject, ConfigObject configObject)
        {
            ...
            catalog.HierarchyType = (HierarchyType)configObject.GetInt32(new int[] { 1, 36 });
            catalog.IsHierarchical = configObject.GetInt32(new int[] { 1, 37 }) != 0;
            ...
            if (catalog.Owners == 1)
            {
                ownerUuid = configObject.GetUuid(new int[] { 1, 12, 2, 2, 1 });
            }
            if (catalog.Owners > 0)
            {
            ...
            ConfigObject properties = configObject.GetObject(new int[] { 6 });
